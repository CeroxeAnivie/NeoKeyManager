...
public class KeyHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) {
        try {
            ...
            if (!SecurityManager.constantTimeEquals(auth, "Bearer " + Config.AUTH_TOKEN)) {
            ...
            if (!signature.valid()) {
                sendResponse(exchange, 401, new ApiError("Unauthorized", signature.reason(), null));
                return;
            }
    ...
    private void handleClientUpdateUrl(HttpExchange exchange) throws IOException {
        ...
        if (os == null || os.isBlank() || serial == null || serial.isBlank() || nodeId == null || nodeId.isBlank()) {
        ...
        String url = null;
        if ("exe".equalsIgnoreCase(os)) {
            url = Config.CLIENT_UPDATE_URL_EXE;
        } else if ("jar".equalsIgnoreCase(os)) {
            url = Config.CLIENT_UPDATE_URL_JAR;
        }

        if (url == null || url.isBlank()) {
            ServerLogger.warnWithSource("API", "nkm.api.updateUrlMissing", os);
            sendResponse(exchange, 200, new DTOs.UpdateUrlResponse(null, false));
        } else {
    ...
    private void handleGetKey(HttpExchange exchange) throws IOException {
        ...
        if (requestName == null || requestName.isBlank() || nodeId == null || nodeId.isBlank()) {
        ...
        SessionManager.PortReservation reservation = SessionManager.getInstance()
                .reserveFirstFreePort(realKeyName, requestName, nodeId, portConfig, maxConns, isSingle);
        if (!reservation.success()) {
            sendResponse(exchange, 409, new ApiError("Too Many Connections", reservation.reason(), KeyStatus.ENABLED));
            return;
        }
    ...
    private void handleHeartbeat(HttpExchange exchange, String body) throws IOException {
        Protocol.HeartbeatPayload payload = Utils.parseJson(body, Protocol.HeartbeatPayload.class);
        if (payload == null || payload.serial == null || payload.serial.isBlank()
                || payload.nodeId == null || payload.nodeId.isBlank()
                || payload.port == null || payload.port.isBlank()) {
    ...
    private void handleSync(HttpExchange exchange, String body) throws IOException {
        Protocol.SyncPayload payload = Utils.parseJson(body, Protocol.SyncPayload.class);
        if (payload == null || payload.nodeId == null || payload.nodeId.isBlank()) {
            sendResponse(exchange, 400, new ApiError("Invalid Payload", "Missing nodeId", null));
            return;
        }
        ...
        java.util.Set<String> metadataRealKeys = new java.util.HashSet<>();

        for (Map.Entry<String, Double> entry : traffic.entrySet()) {
            String alias = entry.getKey();
            Double v = entry.getValue();
            if (alias == null || alias.isBlank() || v == null || !Double.isFinite(v) || v < 0) {
                sendResponse(exchange, 400, new ApiError("Invalid Payload", "Invalid traffic value for key: " + alias, null));
                return;
            }
            String real = Database.getRealKeyName(alias);
            if (real != null) {
                aliasToReal.put(alias, real);
                metadataRealKeys.add(real);
                if (v > 0) {
        ...
        var realMetadataMap = Database.getBatchKeyMetadata(metadataRealKeys.stream().toList());
    ...
    private void handleRelease(HttpExchange exchange, String body) throws IOException {
    ...
    private void handleNodeStatus(HttpExchange exchange, String body) throws IOException {
